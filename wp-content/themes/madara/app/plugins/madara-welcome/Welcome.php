	...
	class Welcome {
		...
		public static function welcome_page_content() {
            ...
            <div class="wrap">
                ...
                if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST' && isset($_POST['validate_envato'])){
                    if(isset($_POST['envato_code']) && $_POST['validate_envato'] == 'activate'){
                        $valid = true;
                        $err = '';
                        ...
                        if(!isset($_POST['madara-registration-terms'])){
                            ...
                            $result = $core->do_validate( $envato_code );
                            ...
                            if(empty($err)){
                                ...
                                if($result){                        
                                    if($data['status'] == 'valid'){
                                        // activate theme
                                        $core = Madara::getInstance();
                                        $core->activate_theme( $envato_code, $data['data']['supported_until'] );                                        
                                    } else {
                                        $err = $data['message'];
                                    }
                                } else {
                        ...
                        if(empty($err) && isset($result['data']) && $result['data']['status'] == 'valid'){
                ...
                if($validate){
                                        ...
                                        <div class="col col-md-5 col-lg-3">
                                            <div class="madara-welcome-item">
                                                ...
                                                <form id="madara-deactivate-form" method="post" action="admin.php?page=madara-welcome">
                                                    ...
                                                    <?php }?>
                                                    <button onclick="return confirm('<?php echo esc_html__('Are you sure?','Madara)');?>')" type="submit" class="button button-secondary" name="validate_envato" value="deactivate"><?php esc_html_e('Deactivate', 'madara');?></button>
                    ...
                    <div style="padding:15px" id="madara-registration-form">
                        ...
                        <form method="post" action="admin.php?page=madara-welcome">
                            <p>
                                <label><?php esc_html_e('Item Purchase Code:', 'madara');?><input type="text" id="envato_code" name="envato_code" value="<?php echo isset($envato_code) ? esc_attr($envato_code) : '';?>"/></label> 
                            </p>
                            <?php if(isset($err) && $err){?>
                            <p class="error"><?php echo esc_html($err);?></p>
                            <?php }?>
                            <p class="desc"><?php esc_html_e('This is the Code you received when you purchase our theme on Envato (ThemeForest.net)','madara');?></p>
                            <p> <button type="submit" class="button button-primary" name="validate_envato" value="activate"><?php esc_html_e('Register', 'madara');?></button></p>
